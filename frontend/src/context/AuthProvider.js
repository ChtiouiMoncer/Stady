...
export const AuthProvider = ({ children }) => { //Exports a component called AuthProvider with a a prop called children, which represents the child components that will be wrapped by this provider.
    ...
    return ( 
        ...
        <AuthContext.Provider value={{ auth, setAuth, isLoggedIn, setIsLoggedIn }}> {/* This makes the auth state variable and its setter function available to all components nested inside the provider */}
            {children}
