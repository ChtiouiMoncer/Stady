import { Alert, Box, Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Grid, ImageList, ImageListItem, ImageListItemBar, List, ListItem, Paper, Snackbar, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography } from "@mui/material";
...
const UnresolvedPitches = () => {
    ...
    const PITCHES_URL = `/api/grounds?page=${page}&isPending=true`;
    ...
    const handleAccept = async (id) => {
        ...
        try {
          ...
          setSuccess(true);
          setTimeout(() => {
            navigate('/admin/manage/pitches/approved', {replace: true});
          }, 2000); 
        } catch (err) {
          setIsPending(false);
        }
      ...
      const handleReject = async (id) => {
        ...
        try {
          ...
          setSuccess(true);
          setTimeout(() => {
            navigate('/admin/manage/pitches/rejected', {replace: true});
        }, 2000); 
        } catch (err) {
          setIsPending(false);
        }
            ...
            renderCell: (params) => (
              <>
                <IconButton color="primary" onClick={() => handleAccept(params.row.id)}>
                  {loadingRow === params.row.id ? <CircularProgress size={24} /> : <CheckCircleIcon />}
                </IconButton>
                <IconButton color="secondary" onClick={() => handleReject(params.row.id)}>
                  {loadingRow === params.row.id ? <CircularProgress size={24} /> : <CancelIcon sx={{color:'red'}} />}
                </IconButton>
              </>
            ),
        },
...
export default UnresolvedPitches;