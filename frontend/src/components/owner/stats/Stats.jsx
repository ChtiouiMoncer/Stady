...
const Stats = () => {
    ...
    const [pitchesPendingCount, setPitchesPendingCount] = useState(0);
    const [pitchesRejectedCount, setPitchesRejectedCount] = useState(0);
    const [pitchesApprovedCount, setPitchesApprovedCount] = useState(0);
    ...
    const getPitches = async () => {
      const PITCHES_URL = `/api/grounds?owner.username=${auth?.username}`;
    ...
    const getPitchesPending = async () => {
      const PITCHES_URL = `/api/grounds?isPending=true&owner.username=${auth?.username}`;
            ...
            try {
              const response = await axiosPrivate.get(PITCHES_URL, {
                ...
                if (isMounted) {
                  setPitchesPendingCount(response.data.length);
                  setPitchesData(response.data); 
    ...
    const getPitchesApproved = async () => {
      const PITCHES_URL = `/api/grounds?isApproved=true&owner.username=${auth?.username}`;
            ...
            try {
              const response = await axiosPrivate.get(PITCHES_URL, {
                ...
                if (isMounted) {
                  setPitchesApprovedCount(response.data.length);
                  setPitchesData(response.data); 
    ...
    const getPitchesRejected = async () => {
      const PITCHES_URL = `/api/grounds?isRejected=true&owner.username=${auth?.username}`;
            ...
            try {
              const response = await axiosPrivate.get(PITCHES_URL, {
                ...
                if (isMounted) {
                  setPitchesRejectedCount(response.data.length);
                  setPitchesData(response.data); 
    ...
    return ( 
      ...
      >
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Typography variant="h6" textAlign="left" sx={{ color: "green.light", marginBottom: '10px' }}> Pitches Stats</Typography>
        </Box>
        <Typography variant="h5" textAlign="center" sx={{ color: "grey.black", marginBottom: '10px' }}> Status Overview </Typography>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-around", flexWrap: "wrap" }}>
          <Grid container spacing={2}>
            {[{
              ...
              count: pitchesApprovedCount
            }, {
              title: "Pending Pitches",
              count: pitchesPendingCount
            }, {
              title: "Rejected Pitches",
              count: pitchesRejectedCount
            }].map((item, index) => (
        ...
        <Box sx={{ display: "flex", justifyContent: "space-around", flexWrap: "wrap" }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={8} sx={{ margin: '5px' }}>
              <Doughnut
                data={data}
                options={{
                  responsive: true,
