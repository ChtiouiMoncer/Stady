...
const OwnerAllPitches = () => {
    ...
    try {
        ...
        const response = await axios.get(`/api/grounds?page=1&isApproved=true&owner.username=${auth?.username}`);
...
useEffect(() => {
    ...
    return ( 
            ...
            {!isPendingPitches ? (
            <><StyledModal>
                    <Box sx={{ marginTop: '15px',  display: 'flex', justifyContent: 'left', alignItems: 'left', flexDirection: 'left', textAlign:'left' }}>
                        {auth && (
                            
                        <Typography variant="h6" sx={{  color: 'green.main' }}>
                            Pitches Owned by {auth.username}
                        </Typography>
                        )}                    
                    </Box>
                    <Box
                                ...
                                return (
                                    <Grid item xs={12} sm={6} md={4} key={index}>
                                        ...
                                        <Card
                                            ...
                                            onClick={() => navigate(`/owner/manage/timeslots/${pitch.name}`, { state: { pitchId: pitch.id } })}
                                        >
                                        ...
                                        <Button variant="contained" onClick={() => navigate(`/owner/update/${pitch.name}`, { state: { pitchId: pitch.id } })}>
                                            Update
                                        </Button>
                                        </Box>
                                        </Box>
...
export default OwnerAllPitches ;